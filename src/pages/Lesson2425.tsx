...
export default function Lesson2425() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const ammeterVal = circuitOk && ammeterConn === "series" ? 0.5 : 0;
      const voltmeterVal = circuitOk && voltmeterConn === "parallel" ? 3 : 0;
      ...
      ctx.lineWidth = 2.5;
      // Top wire: battery+ to bulb
      ctx.beginPath();
      ctx.moveTo(bx + 34, by);
      ctx.lineTo(lx - 25, by);
      ctx.stroke();
      // Bottom wire: bulb to battery-
      ctx.beginPath();
      ctx.moveTo(lx, ly + 24);
      ctx.lineTo(lx, 300);
      ctx.lineTo(bx, 300);
      ...
      const needleA = isRunning ? (ammeterConn === "parallel" ? 1.0 : ammeterVal / 1.0) : 0;
      drawMeter(amX, amY, "A", ammeterVal.toFixed(1), "A", needleA, "#3b82f6");

      // Ammeter connection indicator
      ...
      const needleV = isRunning ? voltmeterVal / 5.0 : 0;
      drawMeter(vmX, vmY, "V", voltmeterVal.toFixed(0), "V", needleV, "#22c55e");
      ctx.fillStyle = voltmeterConn === "parallel" ? "#22c55e" : "#ef4444";
      ctx.font = "9px 'Space Grotesk'";
      ctx.fillText(voltmeterConn === "parallel" ? "Song song ✓" : "Nối tiếp ✗", vmX, vmY + 58);
      ...
      // Warning flash for short circuit
      ...
      ctx.textAlign = "center";
      ctx.fillText("Dùng nút bên dưới để thay đổi cách mắc Ampe kế / Vôn kế", W / 2, H - 15);

      animRef.current = requestAnimationFrame(draw);
    };
    draw();
    return () => cancelAnimationFrame(animRef.current);
  }, [isRunning, ammeterConn, voltmeterConn, showParticles]);
  ...
  return (
    <div className="space-y-4">
      <LessonHeader icon={Gauge} title="Bài 24-25: Cường độ & Hiệu điện thế" subtitle="Đo cường độ dòng điện và hiệu điện thế">
        <ControlPanel
          isRunning={isRunning}
          onToggleRun={() => { setIsRunning(!isRunning); setWarned(false); }}
          onReset={reset}
      ...
      <div className="flex flex-wrap gap-2">
        <button
          onClick={() => setAmmeterConn(ammeterConn === "series" ? "parallel" : "series")}
        ...
        <button
          onClick={() => setVoltmeterConn(voltmeterConn === "parallel" ? "none" : "parallel")}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="glass-panel p-4">
          ...
          <p className="text-3xl font-bold font-mono text-primary">
            {isRunning && ammeterConn === "series" ? "0.5" : "0"}<span className="text-lg ml-1">A</span>
          </p>
        </div>
        <div className="glass-panel p-4">
          <p className="text-xs text-muted-foreground mb-1">Hiệu điện thế (U)</p>
          <p className="text-3xl font-bold font-mono text-success">
            {isRunning && voltmeterConn === "parallel" && ammeterConn !== "parallel" ? "3" : "0"}<span className="text-lg ml-1">V</span>
          </p>
        </div>
      </div>
    </div>
